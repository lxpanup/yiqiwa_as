 ...
 *  预约列表商品
 */
public class OrderBuySellListAdapter extends RecyclerView.Adapter<OrderBuySellListAdapter.MyHolder> {
    private Context context;
    private MainActivity activity;
    public List<MacOrderSubPo> mList;
    private OnClickGengjing onClickGengjing;

    public OrderBuySellListAdapter(Context context, List<MacOrderSubPo> mList, OnClickGengjing onClickGengjing) {
    ...
    public void onBindViewHolder(@NonNull MyHolder holder, int position) {
        ...
        if(macRentOutPoBean.getStatus().equals("3")){
            ...
            holder.txt_cart_status.setTextColor(context.getResources().getColor(R.color.colorPrimary));
            holder.txt_cart_status.setText("跟进中");
        }else if(macRentOutPoBean.getStatus().equals("1")){
            holder.txt_cart_status.setTextColor(context.getResources().getColor(R.color.font_orange_color));
            holder.txt_cart_status.setText("预约中");
